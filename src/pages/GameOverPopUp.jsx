          ...
          <div className={`fixed inset-0 flex items-center justify-center z-50 ${isLoaded ? "loaded" : ""}`}>
            ...
            <div className="pixel3 relative z-50 bg-white px-4 pt-4 pb-6 text-black space-y-2">
                ...
                <div className='mt-2 mx-[7px] flex justify-center items-center space-x-3'>
                    <input type='text' readOnly value={link} className='bg-white ring-2 w-full ring-black rounded-sm '/>
                    <FiCopy className='scale-[1.25] cursor-pointer' onClick={() => navigator.clipboard.writeText(link)}/>
                    <a href={link} className='visited:text-black'><FaSquareXTwitter className='scale-[2] text-black cursor-pointer'/></a>
                </div>
